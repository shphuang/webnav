...
function renderSites() {
    ...
    // 过滤并排序当前分类的网站
    const currentSites = sites.filter(site => site.categoryId === currentCategoryId)
        ...
        siteCard.addEventListener('drop', function(e) {
            ...
            if (draggedSiteIndex !== -1) {
                ...
                siteCards.forEach((card, index) => {
                    const siteId = parseInt(card.querySelector('.site-actions i').dataset.id);
...
function addSite() {
    ...
    if (name && url) {
        ...
        const currentSites = sites.filter(site => site.categoryId === currentCategoryId);
        const maxOrder = currentSites.length > 0 ? Math.max(...currentSites.map(s => s.order)) : -1;
